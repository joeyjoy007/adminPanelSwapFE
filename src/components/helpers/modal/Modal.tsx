...
console.log("openen",user);
  ...
  const deleteUpload = async(fileName: any,id: any)=>{
    try {
      const deleteUploads = await deleteCompleteFile({_id:id._id,fileName:fileName}).then((response: any)=>{
        setOpen(!open)
        messageApi.success({
          type:"success",
...
{contextHolder}
    ...
    <div>  {user?.doneWork?.map((e)=>{
        return <div style={{display:"flex",marginTop:"1rem"}}>
          ...
          <div>
          <Button style={{fontSize:14}} onClick={() =>deleteUpload(e,user) } icon={  <DeleteOutlined style={{color:"black",fontWeight:"bold",fontSize:14}} />}/>
